FROM circleci/ruby:2.5.3-stretch-node-browsers
...
RUN mkdir /tmp/src
RUN ["/bin/bash", "-c", "set -o pipefail && curl http://www.freetds.org/files/stable/freetds-1.00.109.tar.gz | tar xvz -C /tmp/src/"]
RUN cd /tmp/src/freetds-1.00.109 && ./configure --prefix=/usr/local --with-tdsver=7.3
RUN cd /tmp/src/freetds-1.00.109 && make
RUN cd /tmp/src/freetds-1.00.109 && make install